...
def get_status():
    ...
    return jsonify({
        "is_running": xdp_filter.is_running,
        "device": xdp_filter.device,
...
def start_filter():
    ...
    if xdp_filter.is_running:
        return jsonify({"success": False, "message": "Filter is already running"}), 400
        
    success = xdp_filter.start()
    if success:
        return jsonify({"success": True, "message": f"Attached XDP to {xdp_filter.device}"})
    else:
        return jsonify({"success": False, "message": "Failed to attach program. Ensure you are running as root and python3-bpfcc is installed."}), 500

@app.route("/api/stop", methods=["POST"])
def stop_filter():
    """Stops the XDP filter."""
    if not xdp_filter.is_running:
        return jsonify({"success": False, "message": "Filter is not running"}), 400
        
    success = xdp_filter.stop()
    if success:
        return jsonify({"success": True, "message": "Detached XDP program"})
    else:
        return jsonify({"success": False, "message": "Failed to detach program."}), 500
